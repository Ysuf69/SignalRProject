...
namespace SignalRWebUI.Controllers
{
    public class SliderController : Controller
    {
        ...
        public async Task<IActionResult> CreateSlider(CreateSliderDto createSliderDto)
        {
            ...
            var responseMesssage = await client.PostAsync("https://localhost:7155/api/Slider", stringContent);
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7155/api/Slider/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"https://localhost:7155/api/Slider/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
